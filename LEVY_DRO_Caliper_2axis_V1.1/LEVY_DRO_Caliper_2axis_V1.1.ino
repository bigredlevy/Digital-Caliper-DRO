...
//
// 2-axis Code adapted from 3 axis code
// X axis is displayed as diameter (DRO displays 2 x caliper movement)
// Resolution of caliper is therefore reduced to 0.02mm
...
void loop() {
  ...
  if ((micros() - tempTime) > 500) {                  //if HIGH pulse >500 micros, start of a new bit sequence
    ...
    positionAbsX = caliperValue * caliperValueSign * 2;   //calculate absolute position, multiply by 2 for diameter
  ...
  if (key != NO_KEY) {                                //if any button pressed:
    ...
    if (userInputFlagX == false && userInputFlagY == false && userInputFlagZ == false) {    //if no flags true:
      ...
      if (key == 'B') userInputFlagZ = true;            //if "B" (Z) button pressed, set user input flag Z
    }
